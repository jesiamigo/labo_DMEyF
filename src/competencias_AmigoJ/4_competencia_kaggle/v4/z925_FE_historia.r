...
PARAM  <- list()
PARAM$experimento <- "FE9250_competencia_4_v4"
...
TendenciaYmuchomas  <- function( dataset, cols, ventana=5, tendencia=TRUE, minimo=TRUE, maximo=TRUE, promedio=TRUE, 
...
AgregaVarRandomForest  <- function( num.trees, max.depth, min.node.size, mtry)
{
  ...
  dataset_rf[ , entrenamiento := as.integer( foto_mes>= 201901 &  foto_mes<= 201907 & ( clase01==1 | azar < 0.10 )) ]
